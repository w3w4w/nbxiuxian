...
def download_xiuxian_data():
    ...
    # 1. 修改下载地址为新的tag地址对应的资源包（需确认该tag下的xiuxian.zip实际下载链接）
    # 注意：GitHub的tag页面不是直接下载链接，需替换为实际的asset下载链接
    XIUXIAN_ZIP_URL = "https://github.com/w3w4w/nbxiuxian/releases/download/v0/xiuxian.zip"
    ...
    try:
        ...
        if not success:
            ...
            try:
                response = requests.get(XIUXIAN_ZIP_URL, stream=True, timeout=30, verify=False)
...
class UpdateManager:
    def __init__(self):
        self.repo_owner = "w3w4w"
        self.repo_name = "nbxiuxian"
    ...
    def download_release(self, release_tag, asset_name="project.tar.gz"):
        ...
        try:
            # 2. 调整指定tag的Release请求地址（适配新仓库）
            release_url = f"{self.api_url}/tags/{release_tag}"
            response = requests.get(release_url, timeout=10, verify=False)
            response.raise_for_status()
    ...
    def test_proxies(self, proxy_list):
        ...
        def test_proxy(proxy):
            ...
            try:
                ...
                # 3. 测试地址适配新仓库（仅修改域名路径，不影响代理测试逻辑）
                test_url = f"{proxy['url']}https://github.com/w3w4w/nbxiuxian/robots.txt"
    ...
    def download_with_proxy(self, original_url, download_path, proxy):
        ...
        try:
            ...
            # 使用requests下载，支持进度显示
            response = requests.get(proxy_url, stream=True, timeout=30, verify=False)
            response.raise_for_status()
